...
if __name__ == '__main__':
    ...
    if reconfigure:
        ...
        lmTextclassifier=LMTextClassifier();
        lmTextclassifier.load_label_descriptions() # traverse train ,test, dev metadata
        LOGGER.info('\n---------------- Starting ----------------')

        with open("data/models/LMTextClassifier.pickle", "wb") as f:
            pickle.dump(lmTextclassifier, f)
        ...
        # documents_dev = classifier.load_dataset('dev') # traverse train ,test, dev metadata
        # with open("data/models/documents_dev.pickle", "wb") as f:
        #     pickle.dump(documents_dev, f)# len(documents_dev)=6000
    else:
        with open("data/models/documents_dev.pickle", "rb") as f:
            documents_dev=pickle.load(f)# len(documents_dev)=6000
        with open("data/models/LMTextClassifier.pickle", "rb") as f:
            lmTextclassifier=pickle.load(f)# len(documents_dev)=6000


    val_samples, val_tags = lmTextclassifier.process_dataset(documents_dev)
    with open("data/models/val_samples_tags.pickle", "wb") as f:
        pickle.dump((val_samples,val_tags), f)
    pdb.set_trace()
    val_generator = SampleGenerator(val_samples, val_tags, experiment=lmTextclassifier,
                                    batch_size=Configuration['model']['batch_size'])
    pdb.set_trace()
    val_generator.__len__()
    val_generator.__getitem__(0)